...
START_TEST(sprintf_ge_int_more_ten) {
  char str1[SIZE] = {'\0'};
  char str2[SIZE] = {'\0'};

  char format[] = "%.5G %#g";
  double val1 = 999995.5;
  double val2 = 100000000000.123;
  ck_assert_int_eq(s21_sprintf(str1, format, val1, val2),
...
Suite *st_sprintf(void) {
  Suite *s = suite_create("st_sprintf");
  ...
  tcase_add_test(tc, sprintf_hd_plus);
  tcase_add_test(tc, sprintf_ld_negative_num);
  tcase_add_test(tc, sprintf_ge_intpart_andsmth);
  tcase_add_test(tc, sprintf_ge_int_more_ten);
  tcase_add_test(tc, sprintf_ge_accnull1);
  tcase_add_test(tc, sprintf_ge_accnull2);
  tcase_add_test(tc, sprintf_lge_accnull2);
  tcase_add_test(tc, sprintf_lge_1);
  tcase_add_test(tc, sprintf_float_smth);
  tcase_add_test(tc, sprintf_gspec_smth);
  tcase_add_test(tc, sprintf_percent);
  tcase_add_test(tc, sprintf_lge_last_nulls);
  tcase_add_test(tc, sprintf_lho_precision);
  tcase_add_test(tc, sprintf_lhux_precision);
